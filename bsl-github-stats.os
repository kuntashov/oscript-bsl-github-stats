...
Функция ВыполнитьПоиск()

	УникальныеПользователи = Новый Соответствие;
	УникальныеРепо = Новый Соответствие;

	СтрокаПоиска = "КонецПроцедуры OR КонецФункции OR КонецЦикла OR КонецЕсли EndProcedure OR EndFunction in:file extension:os";
	...
	Попытка
		Пока НЕ ПустаяСтрока(СсылкаНаСтраницуРезультатов) Цикл
			...
			Если СчетчикСтраниц % 5 = 0 Тогда
				Сообщить("Ждем 10 секунд...");
				Приостановить(10000);
			КонецЕсли;

			Если СчетчикСтраниц % 10 = 0 Тогда
				Сообщить("Ждем 10 секунд...");
				Приостановить(10000);
			КонецЕсли;
